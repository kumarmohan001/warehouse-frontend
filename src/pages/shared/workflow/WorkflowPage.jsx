...
export default function WorkflowPage({ token, user, title, kind = 'raw', mode, defaultStatus = '', selectedReceiptId, onCloseReceipt = () => {}, onNavigate }) {
  ...
  function saved(updated) { setAction(null); setSelected(updated._id); setReload((value) => value + 1); setToast('Saved successfully. Stock and workflow status are up to date.'); }
  ...
  return <section className="flow-page">
    <div className="page-title"><div><p className="eyebrow">{mode === 'qc' ? 'QUALITY CONTROL' : mode === 'production' ? 'PRODUCTION' : 'WAREHOUSE'}</p><h1>{title}</h1><p>{raw ? 'Track each material batch from receiving through QC and warehouse acceptance.' : 'Every confirmed action records the user, time and material reference.'}</p></div><div className="flow-buttons"><button className="outline" onClick={() => setReload((value) => value + 1)}>Refresh</button>{createAction && <button className="primary" onClick={() => { setSelected(null); setRecord(null); setAction(createAction); }}>{actionNames[createAction]}</button>}</div></div>
    ...
    <article className="stock-list-card"><div className="table-wrap"><table className="stock-table"><thead><tr><th>Reference</th><th>Material / Batch</th><th>Quantity</th><th>Available</th><th>Location / Owner</th><th>Status</th><th>Action</th></tr></thead><tbody>{loading ? <tr><td colSpan="7" className="table-state">Loading records...</td></tr> : records.length ? records.map((item) => <tr key={item._id}><td><b>{item.grnNumber || item.number}</b><small>{item.materialCode}</small></td><td>{item.materialName}<small>{item.batchNo || item.productionOrder || 'Batch pending'}</small></td><td>{item.receivedQuantity ?? item.quantity} {item.quantityUnit}</td><td>{item.status === 'Available' ? `${item.availableQuantity} ${item.quantityUnit}` : 'Blocked / not stock'}</td><td>{item.verification?.location || item.location || item.qcAssignedTo?.name || item.createdBy?.name || 'Not assigned'}</td><td><Status value={item.status} /></td><td><button className="outline" onClick={() => { setSelected(item._id); setAction(null); }}>Open record</button></td></tr>) : <tr><td colSpan="7" className="table-state">No records match this view.</td></tr>}</tbody></table></div><Pager pagination={pagination} onPage={setPage} loading={loading} /></article>
    {(selected || action) && <Modal title={action ? actionNames[action] : `${record?.grnNumber || record?.number || 'Loading...'} · ${record?.materialName || ''}`} busy={busy} onClose={close}>
      {action ? <WorkflowAction key={action + (record?._id || '')} action={action} record={record} token={token} user={user} lookups={lookups} onSaved={saved} onBusy={setBusy} onCancel={() => record ? setAction(null) : close()} /> : !record ? <p>Loading details...</p> : <>
        <div className="flow-record-header"><Status value={record.status} /><div className="flow-buttons">{availableActions.map((name) => <button key={name} className={name === 'sampling' || name === 'acceptRaw' ? 'primary' : 'outline'} onClick={() => setAction(name)}>{actionNames[name]}</button>)}</div></div>
