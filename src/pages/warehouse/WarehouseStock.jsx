...
const editFields = ['supplierName', 'poNumber', 'invoiceNumber', 'batchNo', 'manufacturer', 'receivedQuantity', 'containers', 'manufacturingDate', 'expiryDate', 'receivingDate', 'storageRequirement', 'remarks'];
...
export default function WarehouseStock({ token, onNavigate = () => {}, onCreate, initialToast = '' }) {
  ...
  const loadRecords = async () => {
    ...
    try { const data = await warehouseApi.list(token); setRecords(data.records || []); }
  ...
  function openEdit(record) { setEditing(record); setEditForm(editValues(record)); setDocumentFiles({}); }
  ...
  async function removeRecord(record) {
    if (!window.confirm(`Delete ${record.grnNumber}? This also deletes its Cloudinary documents.`)) return;
    setError('');
    try { await warehouseApi.remove(token, record._id); setRecords((current) => current.filter((item) => item._id !== record._id)); setToast('Stock record deleted successfully.'); }
    catch (requestError) { setError(requestError.message || 'Unable to delete stock record.'); }
  }
  async function saveDocuments() {
    ...
    try {
      ...
      setToast(record.status === 'Quarantine' ? 'Documents updated. All documents are complete, so status changed to Quarantine.' : 'Documents updated. The receipt remains on Document Hold until all documents are uploaded.');
  ...
  return <section className="stock-list-page">
    ...
    <article className="stock-list-card"><div className="card-heading"><div><p className="eyebrow">MATERIAL RECEIPTS</p><h3>{records.length} stock records</h3></div><button className="outline" type="button" onClick={loadRecords} disabled={loading}>↻ Refresh</button></div><div className="table-wrap"><table className="stock-table"><thead><tr><th>GRN</th><th>Material name</th><th>Supplier name</th><th>QC reviewer</th><th>Expiry</th><th>Status</th><th>Actions</th></tr></thead><tbody>{loading ? <tr><td colSpan="7" className="table-state">Loading stock records…</td></tr> : records.length ? records.map((record) => <tr key={record._id}><td><b>{record.grnNumber}</b><small>{record.materialCode}</small></td><td>{record.materialName}</td><td>{record.supplierName}</td><td>{record.qcAssignedTo?.name || 'Not assigned'}</td><td>{displayDate(record.expiryDate)}</td><td><span className={`stock-status ${record.status === 'Quarantine' ? 'quarantine' : 'hold'}`}>{record.status}</span></td><td className="stock-actions"><button type="button" title="View record" aria-label={`View ${record.grnNumber}`} onClick={() => setViewing(record)}>◉</button><button type="button" title="Edit record" aria-label={`Edit ${record.grnNumber}`} onClick={() => openEdit(record)}>✎</button><button className="delete-action" type="button" title="Delete record" aria-label={`Delete ${record.grnNumber}`} onClick={() => removeRecord(record)}>⌫</button></td></tr>) : <tr><td colSpan="7" className="table-state">No stock receipts yet. Use “Fill stock form” to create one.</td></tr>}</tbody></table></div></article>
    ...
    {editing && <div className="user-modal-backdrop" onMouseDown={() => !saving && setEditing(null)}><form className="user-form user-modal stock-edit-form" onSubmit={saveEdit} onMouseDown={(event) => event.stopPropagation()}><div className="card-heading"><div><h3>Edit {editing.grnNumber}</h3><p>{editing.materialName} ({editing.materialCode}) · <b>{editing.status}</b></p></div><button className="modal-close" type="button" onClick={() => !saving && setEditing(null)}>×</button></div><div className="user-fields">{editFields.map((field) => <label key={field}>{field.replace(/([A-Z])/g, ' $1').replace(/^./, (letter) => letter.toUpperCase())}{field === 'remarks' ? <textarea value={editForm[field]} onChange={(event) => setEditForm((values) => ({ ...values, [field]: event.target.value }))} /> : field === 'storageRequirement' ? <select value={editForm[field]} onChange={(event) => setEditForm((values) => ({ ...values, [field]: event.target.value }))}><option>Ambient / Room temperature</option><option>Cool (2–8°C)</option><option>Frozen (-20°C)</option><option>Protect from light</option><option>Dry storage</option></select> : <input required={field !== 'remarks'} type={field.endsWith('Date') ? 'date' : ['receivedQuantity', 'containers'].includes(field) ? 'number' : 'text'} value={editForm[field]} onChange={(event) => setEditForm((values) => ({ ...values, [field]: event.target.value }))} />}</label>)}</div><button className="primary" disabled={saving} type="submit">{saving ? 'Saving...' : 'Save changes'}</button><section className="edit-documents"><h4>Upload / replace documents</h4><p>Upload missing documents. When all four are present, status changes to Quarantine automatically.</p><div className="document-grid">{documentFields.map(([key, label]) => <label className="document-upload" key={key}><span>{label}</span><input type="file" accept=".pdf,.jpg,.jpeg,.png,.doc,.docx" onChange={(event) => setDocumentFiles((files) => ({ ...files, [key]: event.target.files[0] }))} /><small>{documentFiles[key]?.name || (editing.documents?.[key === 'invoiceDocument' ? 'invoice' : key === 'otherDocuments' ? 'otherRequiredDocuments' : key]?.fileName || 'No file selected')}</small></label>)}</div><button className="outline" type="button" disabled={saving} onClick={saveDocuments}>{saving ? 'Uploading...' : 'Upload documents'}</button></section></form></div>}
  </section>;
}
